...
function initTestData (dataUrlPart, appNm, appUrlPattern) {
  ...
  waitForVisible('body pre');

  listOfFavorites = JSON.parse(browser.getHTML('body pre', false))[dataUrlPart];
  appName = appNm;
...
Then(/^every favorite should open without errors$/, { timeout: 1000 * 1000 }, () => {
  ...
  listOfFavorites.forEach((favorite) => {
    ...
    const dataExist = !browser.element('//*[contains(translate(text(), "No", "no"), "no data")]').isExisting();
    ...
    allure.createStep(favorite.displayName, reportLog, 'attachment', status);
    if (status === 'failed') {
